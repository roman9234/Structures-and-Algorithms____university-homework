...
# herbivore_min_breeding_energy = 40
herbivore_min_breeding_energy = 400

herbivore_breeding_cost = 10

herbivore_energy_for_eating_a_tree = 10
...
predator_min_movement_distance = 2
predator_max_movement_distance = 2
        ...
        for x in range(len(_field[y])):
            ...
            if q > none_probability:
                if q <= 1 - herbivore_probability - predator_probability:
                    ...
                    new_herb = [0] * AMOUNT_OF_STATUSES
                    new_herb[TYPE] = HERBIVORE_TYPE
                    new_herb[ENERGY] = herbivore_starting_energy
                    new_herb[STATUS_ACTION] = STATUS_ACTION_READY
                    new_herb[STATUS_DIRECTION] = STATUS_DIRECTION_NO_DIRECTION
                    res = new_herb
                else:
                    res = [PREDATOR_TYPE, predator_starting_energy]
            _field[y][x] = res
                                            ...
                                            if y > 0:
                                                if _field[y - 1][x] == 0:
                                                    if can_breed:
                                                        ...
                                                        _field[y - 1][x] = new_herb
                                                        _new_field[y - 1][x] = new_herb
